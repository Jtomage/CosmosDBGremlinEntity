...
namespace Datalayer.Gremlin
{
  public class UserRepository : RepositoryBase, IUserRepository
  {
    ...
    public Task<Vertex<UserProperties>> CreateUser(Vertex<UserProperties> user)
    {
      //Build the query since cosmos db does NOT support the fluent api https://github.com/Azure/azure-cosmos-dotnet-v2/issues/439
      StringBuilder sb = new StringBuilder("g");
      sb.Append(".addV('User')");
      sb.Append(".property('Version', version)");
      sb.Append(".property('FirstName', firstName)");
      sb.Append(".property('LastName', lastName)");
      sb.Append(".property('SecurityStamp', securityStamp)");
      sb.Append(".property('CreatedDate', createdDate)");
      ...
      Dictionary<string, object> arguments = new Dictionary<string, object>
      {
        { "version", user.Properties.Version },
        { "firstName", user.Properties.FirstName },
        { "lastName", user.Properties.LastName },
        { "securityStamp", user.Properties.SecurityStamp },
        { "createdDate", DateTime.UtcNow }
      };

      var result = ExecuteQueryAndSerialize(query, arguments).GetAwaiter().GetResult();
      var userReturn = JsonConvert.DeserializeObject<Vertex<UserProperties>>(result);
      return Task.FromResult(userReturn);
    }
